...
import { roleLabels,roleOptions,roleDescriptions,displayLogin } from './accessConfig';
import { useAccessSettings } from './useAccessSettings';
import { DingTalkRoleMappings } from './DingTalkRoleMappings';
import './access.css';
export function AccessSettingsPage({users}:{users:UserProfile[]}){
  ...
  const rolesPanel=<div className="access-role-grid">{roleOptions.map(({value,label})=>{const info=roleDescriptions[value as RoleKey];return <Card key={value} size="small" title={label}><dl><dt>数据范围</dt><dd>{info.scope}</dd><dt>允许操作</dt><dd>{info.abilities}</dd><dt>操作限制</dt><dd>{info.limits}</dd></dl></Card>;})}</div>;
  const employeePanel=<div className="access-section">
    <div className="access-section-heading"><div><h3>本店员工 <span className="access-count">{c.users.length}</span></h3><p>这里仅展示系统识别结果，不在此给员工分配角色。管理员在钉钉调整岗位后，员工重新进入应用时自动同步。</p></div></div>
  ...
  return <section className="access-page" aria-label="权限设置">
    <div className="access-heading"><div><h2>钉钉权限对接</h2><p>部门和角色在钉钉分配；员工打开应用后自动识别身份，无需手动登录。</p></div><Button loading={c.loading} disabled={c.saving} onClick={()=>void c.refresh()}>刷新权限</Button></div>
    ...
    {c.notice?<Alert type="success" showIcon title={c.notice} closable onClose={()=>c.setNotice('')}/>:null}
    <Card className="access-main-card"><Tabs items={[
      {key:'employees',label:'员工识别结果',children:employeePanel},
      {key:'roles',label:'页面与权限',children:<div className="access-section"><div className="access-section-heading"><div><h3>四种固定角色</h3><p>服务顾问承担派单与工时录入职责。权限由系统统一定义。</p></div></div>{rolesPanel}<Alert type="info" showIcon title="多角色只生效一个" description="优先级：门店管理员 → 服务顾问 / 派单 → 检验员 → 维修技师。不是将多个角色的权限相加。"/></div>},
      {key:'sync',label:'对接规则',children:<DingTalkRoleMappings controller={c}/>}
    ]}/></Card>
    <Drawer open={Boolean(employee)} onClose={()=>setSelected(undefined)} title={employee?`${employee.name} · 当前权限`:'员工权限'} size={mobile?'100%':480}>
      {employee?<div className="access-section"><Space wrap><Tag color="blue">{roleLabels[employee.role]}</Tag>{statusTag(employee)}</Space>
        ...
        <Collapse items={[{key:'change',label:'如何调整员工权限？',children:<ol className="access-steps"><li>在钉钉中调整员工所属角色。</li><li>在“对接规则”确认角色授权与部门归属规则。</li><li>员工重新进入钉钉应用后自动更新，再刷新本页核对。</li></ol>}]}/>
      </div>:null}
    </Drawer>
  </section>;
}
